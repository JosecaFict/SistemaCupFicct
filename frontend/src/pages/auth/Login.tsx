...
export function Login() {
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      ...
      <Input label="Contrasena" type="password" autoComplete="current-password"
             value={password} onChange={(e) => setPassword(e.target.value)} required />

      <Button type="submit" loading={cargando} className="w-full">Ingresar</Button>

      <div className="flex justify-between text-sm">
        <Link to="/forgot-password" className="text-institutional-700 hover:underline">Olvide mi contrasena</Link>
